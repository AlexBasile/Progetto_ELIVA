...
elseif mode == 1
    [A1, dA1] = create_descriptor(centers1(:,ind1), assignments, fi);
else
    [A1, dA1] = create_descriptor(centers1(:,ind1), assignments, fi,di);
end;
...
for i=1:num_images
    ...
    if mode==0
        [centers2, assignments] = SIFT_AKULA(Images{1,sel_img},canny,num_cluster);
    else
        [centers2, assignments, fi, di] = SIFT_AKULA(Images{1,sel_img},canny,num_cluster);
    end;
    ...
    elseif mode == 1
        [A2, dA2] = create_descriptor(centers2(:,ind2), assignments, fi);
    else
        [A2, dA2] = create_descriptor(centers2(:,ind2), assignments, fi,di);
    end;
    if mode==0
