function [sim,d2] = AKULA_Sim( A1,A2, dA1, dA2 )
%AKULA_SIM
...
d_akula = (distA1+distA2) / num_cluster;

d1 = (distA1/sum(A1(3,:))+distA2/sum(A2(3,:))) / num_cluster/max_dist;
d2=0;
if complete
    d2=sum(sum(d_w)) / (num_cluster^2);
end
%sim = p*d1 + (1-p)*d2;
sim = d_akula;
end
